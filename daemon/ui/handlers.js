      ...
      try {
        ...
        const res = await copperFetch(target, {
          ...
          headers: {
            ...
            'x-copper-token': model.authToken
          },
    ...
    const reloadExtensionsBtn = document.getElementById('reloadExtensionsBtn');
    if (reloadExtensionsBtn) {
      reloadExtensionsBtn.addEventListener('click', async () => {
        try {
          await refreshDescriptorModel();
          renderNav();
          await renderSection();
          setStatus('Extensions reloaded.');
        } catch (err) {
